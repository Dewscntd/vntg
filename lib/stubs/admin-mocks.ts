  ...
  async getDashboardStats() {
    ...
    return {
      ...
      recent_activity: [
        ...
        {
          type: 'customer',
          message: 'New customer registration: jane.doe@example.com',
          timestamp: new Date(now.getTime() - 15 * 60 * 1000),
        },
        {
          type: 'product',
          message: 'Product "iPhone 15 Pro" low stock alert (5 remaining)',
      ...
      top_products: [
        ...
        { id: 'prod-007', name: 'Nike Air Force 1', sales: 89, revenue: 9790.0 },
  ...
  async getAllProducts(filters = {}, pagination = { page: 1, limit: 20 }) {
    ...
    if ((filters as any).category) {
      filteredProducts = filteredProducts.filter(
        (p) => p.category_id === (filters as any).category
      );
    }
    ...
    if ((filters as any).stock_status) {
      filteredProducts = filteredProducts.filter((p) => {
        switch ((filters as any).stock_status) {
          case 'in_stock':
            return p.inventory_count > 0;
          case 'low_stock':
            return p.inventory_count > 0 && p.inventory_count <= 10;
          case 'out_of_stock':
            return p.inventory_count === 0;
          default:
            return true;
...
export class MockInventoryManager {
  ...
  async getInventoryLevels(filters = {}) {
    ...
    if ((filters as any).low_stock) {
      inventoryItems = inventoryItems.filter((item) => item.current_stock <= item.reorder_point);
    }

    if ((filters as any).out_of_stock) {
      inventoryItems = inventoryItems.filter((item) => item.current_stock === 0);
    }
