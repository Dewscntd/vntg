    ...
    select: (columns: string = '*') => {
      const queryBuilder = {
        ...
        async then(resolve: Function) {
          ...
          if (this._single) {
            resolve({ data: data[0] || null, error: data[0] ? null : { message: 'No rows found' } });
          } else {
          ...
          single: async () => {
            ...
            switch (table) {
              case 'products':
                ...
                if (productIndex !== -1) {
                  mockProducts[productIndex] = { ...mockProducts[productIndex], ...data, updated_at: new Date().toISOString() };
  ...
  channel: (name: string) => ({
    on: (event: string, filter: any, callback: Function) => {
      ...
      const interval = setInterval(() => {
        if (Math.random() < 0.1) { // 10% chance of update every 5 seconds
          callback({
            eventType: event,
