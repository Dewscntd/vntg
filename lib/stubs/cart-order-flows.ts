...
class MockOrderManager {
  ...
  async createOrder(userId: string, cartData: any, shippingAddress: any, billingAddress: any, paymentMethod: any) {
...
export class MockCheckoutFlow {
  ...
  async calculateShipping(cartData: any, address: any, method = 'standard') {
    ...
    const weight = cartData.items.reduce((total: number, item: any) => 
      total + (item.product.weight || 1) * item.quantity, 0
    );
    ...
    const baseMethod = shippingMethods[method as keyof typeof shippingMethods] || shippingMethods.standard;
  ...
  async completeCheckout(userId: string, checkoutData: any) {
    try {
      ...
      const shipping = await this.calculateShipping(cart, checkoutData.shipping_address, checkoutData.shipping_method);
  ...
  get cart() { return this.cartManager; }
  get orders() { return this.orderManager; }
}
