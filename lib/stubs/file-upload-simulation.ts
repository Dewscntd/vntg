...
export class MockFileUploadManager {
  ...
  async uploadFile(file: File | Blob, options: {
    ...
    return new Promise<any>((resolve, reject) => {
      ...
      if (Math.random() < 0.05) { // 5% chance of failure
        ...
        }, Math.random() * 2000 + 1000); // Fail after 1-3 seconds
      }
    });
  }
  ...
  private async validateFile(file: File | Blob, rules?: any) {
    ...
    if (rules.maxSize && file.size > rules.maxSize) {
      throw new Error(`File size ${this.formatFileSize(file.size)} exceeds maximum allowed size ${this.formatFileSize(rules.maxSize)}`);
    }
    ...
    if (rules.allowedTypes && file instanceof File) {
      ...
      if (!rules.allowedTypes.includes(fileExtension)) {
        throw new Error(`File type .${fileExtension} is not allowed. Allowed types: ${rules.allowedTypes.join(', ')}`);
      }
    }
  ...
  private async validateImageDimensions(file: File, maxDimensions: { width: number; height: number }) {
    ...
    if (mockDimensions.width > maxDimensions.width || mockDimensions.height > maxDimensions.height) {
      throw new Error(`Image dimensions ${mockDimensions.width}x${mockDimensions.height} exceed maximum allowed ${maxDimensions.width}x${maxDimensions.height}`);
    }
  }
  ...
  private completeUpload(uploadId: string, file: File | Blob, bucket: string, path: string) {
    const fileInfo = {
      ...
      metadata: {
        ...
        height: file.type?.startsWith('image/') ? Math.floor(Math.random() * 1500) + 400 : undefined,
      },
    };
  ...
  async deleteFile(bucket: string, path: string) {
    ...
    // Find file by path
    const file = Array.from(this.uploadedFiles.values()).find(f => 
  ...
  async generateOptimizedUrl(bucket: string, path: string, transformations?: {
    width?: number;
    height?: number;
    quality?: number;
  ...
  getStorageStats(bucket: string) {
    ...
    const filesByType = bucketFiles.reduce((acc, file) => {
