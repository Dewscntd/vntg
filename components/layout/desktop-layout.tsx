...
export function DesktopTwoColumn({
  ...
  const sidebarClasses = cn(
    ...
    stickysidebar &&
      'lg:sticky lg:top-24 lg:self-start lg:max-h-[calc(100vh-6rem)] lg:overflow-y-auto',
  ...
  return (
    <ResponsiveContainer size="xl" className={className}>
      <div className={cn('flex flex-col lg:flex-row', gapClasses[gap])}>
        {sidebarPosition === 'left' && <aside className={sidebarClasses}>{sidebar}</aside>}

        <main className={mainClasses}>{children}</main>

        {sidebarPosition === 'right' && <aside className={sidebarClasses}>{sidebar}</aside>}
      </div>
    </ResponsiveContainer>
  );
}
...
export function DesktopThreeColumn({
  ...
  const sidebarClasses = cn(
    ...
    sidebarWidthClasses[sidebarWidth],
    stickySidebars &&
      'xl:sticky xl:top-24 xl:self-start xl:max-h-[calc(100vh-6rem)] xl:overflow-y-auto'
  );

  return (
    <ResponsiveContainer size="2xl" className={className}>
      <div className={cn('flex flex-col xl:flex-row', gapClasses[gap])}>
        {leftSidebar && <aside className={sidebarClasses}>{leftSidebar}</aside>}

        <main className="min-w-0 flex-1">{children}</main>

        {rightSidebar && <aside className={sidebarClasses}>{rightSidebar}</aside>}
      </div>
    </ResponsiveContainer>
  );
}
...
export function DesktopCardGrid({
  ...
  return (
    <div
      className={cn('grid w-full', gapClasses[gap], equalHeight && 'auto-rows-fr', className)}
      style={{
...
export function DesktopMasonry({
  ...
  return (
    <div className={cn('columns-1', columnClasses, gapClasses[gap], className)}>{children}</div>
  );
}
