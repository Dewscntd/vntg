...
export function ProductSEO({ product, canonical }: ProductSEOProps) {
  ...
  const description = product.description || `Shop ${product.name} at VNTG. Premium quality products at great prices.`;
  ...
  return (
    <Head>
      ...
      <meta name="keywords" content={`${product.name}, ${product.category?.name || 'products'}, VNTG, shop, buy`} />
...
export function CategorySEO({ category, productCount, canonical }: CategorySEOProps) {
  ...
  const description = category.description || `Shop ${category.name} at VNTG. ${productCount ? `${productCount} products` : 'Premium products'} available.`;
